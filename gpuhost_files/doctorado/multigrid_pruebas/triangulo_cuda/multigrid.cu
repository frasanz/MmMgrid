...
void multigrid(Grid *u, Grid *f, Grid *v, Grid *d,int nivel, float *max,int * iter);

void imprime_malla(float * f, int dim, const char * nombre)
{
	FILE * fil;
	fil=fopen(nombre,"w");
	int i,j;
...
void multigrid(Grid *u, Grid *f, Grid *v, Grid *d,int m,float *max,int *iter)
{
	int dim;
	int dim_;
	int i;
	...
	float * a_op;
	cudaMalloc(&a_op,9*sizeof(float));
	cudaMemcpy(a_op,&operador[0],9*sizeof(float),cudaMemcpyHostToDevice);
