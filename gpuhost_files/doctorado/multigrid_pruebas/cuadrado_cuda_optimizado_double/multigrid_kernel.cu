...
{
  __shared__ double bf[BLOCK_DIM][BLOCK_DIM];
  __shared__ double buo[BLOCK_DIM+2][BLOCK_DIM+2];
...
__global__ void defecto(Grid u, Grid f, Grid d)
{
  ...
  __shared__ double bu[BLOCK_DIM+2][BLOCK_DIM+2];
  __shared__ double bf[BLOCK_DIM][BLOCK_DIM];
...
__global__ void suma(Grid u, Grid v)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  int j = blockIdx.y * blockDim.y + threadIdx.y;
  __shared__ double bv[BLOCK_DIM][BLOCK_DIM];
  if(i<u.d && j<u.d)
  {
