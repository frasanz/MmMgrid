...
void multigrid(Grid *u, Grid *f, Grid *v, Grid *d, int m, double * max,double *def, double * hdef);
...
int main()
{
	...
	/* un double para calcular el maximo del defecto de cada fila */
	double * def;  //En la GPU
	double * hdef; //En el host
	...
	dim = (int)pow(2,m+1)+1;
	size=dim*dim;
	cudaMalloc(&def,size*sizeof(double));
	sizetotal+=size;
	hdef=(double*)malloc(size*sizeof(double));

	printf("Necesitamos %d Kb\n",sizetotal*sizeof(double)/1024);
	...
	for(i=0;i<20;i++)
	{
		...
		printf("[Iteracion #%d] nd=%e ratio=%0.10f\n",i,max,max/max_ant);
	}
...
void multigrid(Grid *u, Grid *f, Grid *v, Grid *d, int m, double * max,double *def, double * hdef)
{
	...
	else
	{
		...
		if(m==N_MALLAS-1)
		{
			...
			cudaMemcpy(hdef,def,dim*dim*sizeof(double),cudaMemcpyDeviceToHost);
