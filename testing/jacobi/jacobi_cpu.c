...
void muestra_plot(float**u,float h)
{
	int i,j;
	for(i=0;i<SIZE;i++)
	{
...
int main()
{
	...
	u_new = (float**)malloc(SIZE*SIZE*sizeof(float));
	f     = (float**)malloc(SIZE*SIZE*sizeof(float));

	for(i=0;i<SIZE;i++)
	{
		u[i]    =(float*)malloc(size);
		u_new[i]=(float*)malloc(size);
